...
def tokenize_function(examples):
    return tokenizer(examples[text_column_name], truncation=True, max_length=128, padding='max_length')
...
#  BERT  Linear → DecomposeLinear  
def replace_linear_layer_bert(model, rank=None):
    """
    Recursively replaces Linear layers with DecomposeLinear in BertForSequenceClassification.
    """

    def replace_linear_layer_helper(module, rank=None):
        for name, child in module.named_children():
            if isinstance(child, nn.Linear) and not isinstance(child, DecomposeLinear):
                # Automatically determine rank if not specified
                if rank is None:
                    r, c = tuple(child.weight.size())
                    rank = r * c // (r + c)  # Heuristic rank calculation

                print(f"Replacing {type(module).__name__}.{name} with DecomposeLinear, rank={rank}")
                module._modules[name] = DecomposeLinear(child, rank)

            else:
                replace_linear_layer_helper(child, rank)

    # Replace inside BERT encoder layers
    for i, layer in enumerate(model.bert.encoder.layer):
        replace_linear_layer_helper(layer, rank)

    # Replace inside the pooler (if used)
    if hasattr(model.bert, "pooler"):
        replace_linear_layer_helper(model.bert.pooler, rank)

    # Replace the final classifier
    if isinstance(model.classifier, nn.Linear):
        print(f"Replacing classifier with DecomposeLinear, rank={rank}")
        model.classifier = DecomposeLinear(model.classifier, rank)

    # Optional: Replace lm_head if you're using a BERT with a language modeling head
...
def train_model_llama_gradient_saving(model, tokenizer, optimizer, train_dataloader, eval_dataloader, accelerator, epochs=3, grad_accum_steps=1):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
...
def train_model_bert_gradient_saving(model, tokenizer, optimizer, train_dataloader, eval_dataloader, accelerator, epochs=3, grad_accum_steps=1):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}")):
            ...
            # CrossEntropyLoss (handle padding)
            loss = F.cross_entropy(outputs.logits, batch["labels"], reduction="mean")
            ...
            if (step + 1) % grad_accum_steps == 0:
                ...
                if is_last_step:
                    for i, layer in enumerate(model.bert.encoder.layer):
                        save_gradient(layer.attention.self.query, "query", i)
                        save_gradient(layer.attention.self.key, "key", i)
                        save_gradient(layer.attention.self.value, "value", i)
                        save_gradient(layer.attention.output.dense, "projection", i)
                        save_gradient(layer.intermediate.dense, "fc1", i)
                        save_gradient(layer.output.dense, "fc2", i)
...
def train_model_bert(model, tokenizer, optimizer, train_dataloader, eval_dataloader, accelerator, epochs=3, grad_accum_steps=1):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.train()
...
def apply_noise_to_bert(model, std, th, device="cuda"):
    for i, layer in enumerate(model.bert.encoder.layer):
        modules = [
            ("query", layer.attention.self.query),
            ("key", layer.attention.self.key),
            ("value", layer.attention.self.value),
            ("projection", layer.attention.output.dense),
            ("fc1", layer.intermediate.dense),
            ("fc2", layer.output.dense)
        ]

        for name, sub_layer in modules:
            if isinstance(sub_layer, DecomposeLinear):
                gradients = sub_layer.Sigma.diag.grad
                if gradients is None:
                    print(f"🚨 Warning: No grad for {name} in layer {i}")
                    gradients = torch.zeros_like(sub_layer.Sigma.diag, device=device)
                ...
                # Original Quantized blocks
                original_MSB_U, original_MID_U, original_MID2_U, original_LSB_U = uniform_quantization(sub_layer.U.weight, clip_val_U, 8)
                original_MSB_VS, original_MID_VS, original_MID2_VS, original_LSB_VS = uniform_quantization(
                    torch.diag(sub_layer.Sigma.diag) @ sub_layer.V.weight, clip_val_VS, 8
                )

                # Quantize again for injection
                temp_MSB_U, temp_MID_U, temp_MID2_U, temp_LSB_U = uniform_quantization(sub_layer.U.weight, clip_val_U, 8)
                temp_MSB_VS, temp_MID_VS, temp_MID2_VS, temp_LSB_VS = uniform_quantization(
                    torch.diag(sub_layer.Sigma.diag) @ sub_layer.V.weight, clip_val_VS, 8
                )
                ...
                # Apply reconstructed weights
                sub_layer.U.weight.data = (temp_MSB_U + temp_MID_U + temp_MID2_U + temp_LSB_U).to(device)
                sub_layer.Sigma.diag.data = torch.diag(torch.eye(temp_MSB_U.shape[1])).to(device)  # Identity
                sub_layer.V.weight.data = (temp_MSB_VS + temp_MID_VS + temp_MID2_VS + temp_LSB_VS).to(device)
...
def load_gradients_bert(model):
    device = next(model.parameters()).device  # 현재 모델이 위치한 디바이스
    ...
    print("✅ All Gradients Loaded Successfully for BERT!")